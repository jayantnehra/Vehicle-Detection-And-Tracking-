...
#color_space can be selected from YCrCb RGB, HSV, LUV, HLS, YUV, YCrCb  
color_space = 'RGB'
...
clf_path = 'clf_pickle_all_v1.p'  # CAMERA CALIBRATION FILE
Heatmap_buffer = []
N_buffer = 10
y_start_stop = [400, 656] # Min and MAX in y to search in slide_window()
ystart_0 = y_start_stop[0]
ystop_0 = ystart_0 + 64*3
ystart_1 = ystart_0
...
# COLOUR HISTOGRAM FUNCTION [COMPUTING SEPERATELY FOR EVERY CHANNEL ]
...
# Feature extraction function[call bin_spatial() and color_hist()]
def extract_features(imgs, color_space='RGB', spatial_size=(32, 32),
    ...
    for file in imgs:
        ...
        if hog_feat == True:
            ...
            # Adding the new feature vector to the features list
...
def slide_window(img, x_start_stop=[None, None], y_start_stop=[None, None],
    ...
    # The Number of pixels per step in x/y
    nx_pix_per_step = np.int(xy_window[0]*(1 - xy_overlap[0]))
    ny_pix_per_step = np.int(xy_window[1]*(1 - xy_overlap[1]))
    # The number of windows in x/y
...
# Extracting features from a single image window
def single_img_features(img, color_space='RGB', spatial_size=(32, 32),
    ...
    if hog_feat == True:
        if hog_channel == 'ALL':
            ...
            hog_features = get_hog_features(feature_image[:,:,hog_channel], orient,
                        pix_per_cell, cell_per_block, vis=False, feature_vec=True)
        #Add features to Image feature list
